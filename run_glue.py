...
class AdaLoraCallback(TrainerCallback):
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        # 显式调用 update_and_allocate
        if hasattr(actual_model, "update_and_allocate"):
            actual_model.update_and_allocate(state.global_step)
...
def main():
    ...
    if os.environ.get("USE_PEFT_ADALORA") == "true":
        print(">>> [PEFT] 环境变量触发！正在注入 AdaLoRA 模块...")
        from peft import get_peft_model, AdaLoraConfig, TaskType
        
        mag_alpha_env = os.environ.get("MAG_ALPHA", "1.0")
        current_mag_alpha = float(mag_alpha_env)
        print(f">>> [PEFT] 当前 Magnitude Alpha 设置为: {current_mag_alpha}")
        # =======================================================================
        ...
        peft_config = AdaLoraConfig(
            ...
            magnitude_alpha=current_mag_alpha, 
            
            total_step=int(os.environ.get("TOTAL_STEPS", "10000")),  
            tinit=200,
            tfinal=1000,
            deltaT=10,
        )
    ...
    trainer = Trainer(
        ...
        callbacks=[AdaLoraCallback] if os.environ.get("USE_PEFT_ADALORA") == "true" else None,
    )
